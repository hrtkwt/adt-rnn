...
import librosa
import pandas as pd
peak_params = {"pre_max": 3, "post_max": 3, "pre_avg": 3, "post_avg": 3, "delta": 0.05, "wait": 2}

def peak_picking(activation, pre_max, post_max, pre_avg, post_avg, delta, wait):

    peak = np.zeros(activation.shape, dtype='int32')

    peak_indices = librosa.util.peak_pick(activation, pre_max, post_max, pre_avg, post_avg, delta, wait).astype(np.int32)
...
def peaks(Y_activations, **kwargs):
    ...
    Y_peak = np.hstack((Y_peak_HH[:,np.newaxis], Y_peak_SD[:,np.newaxis], Y_peak_KD[:,np.newaxis]))
...
def accuracy(Y_annotation, Y_pred, pre_tolerance, post_tolerance, dbg=False):
    ...
    return tp, fp, fn, tp+fp, tp+fn, round(precision, 4), round(recall, 4), round(fmeasure, 4)
...
def get_result_table(audio_names):
    ...
    result_table = pd.DataFrame(np.zeros(n_audio * n_metrics * n_inst).reshape(n_audio, n_metrics * n_inst), index = audio_names, columns = [column1, column2])
    result_table.index.names = ['audio']
    result_table.columns.names = ['inst', 'metrics']

    return result_table

Y_peak = peaks(Y_pred, **peak_params)
